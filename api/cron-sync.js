...
export default async function handler(req, res) {
    try {
        ...
        for (const movie of targetMovies) {
            ...
            try {
                const apiRes = await fetch(`${BASE_URL}/phim/${activeSlug}`, { headers: { 'accept': 'application/json' } });
        ...
        return res.status(200).json({ success: true, newEpisodes: totalNew, fixedEpisodes: totalFixed });
    } catch (err) {
