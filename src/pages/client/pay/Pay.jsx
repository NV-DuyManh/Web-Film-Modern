...
function Pay(props) {
    ...
    const comboPlans = useMemo(() => {
        return [...plans]
            .filter(p => Number(p.level) >= moviePlanLevel && Number(p.price) > 0)
            .sort((a, b) => Number(a.level) - Number(b.level));
    }, [plans, moviePlanLevel]);
                            ...
                            <div className="flex flex-col sm:flex-row justify-between sm:items-center relative z-10 gap-4">
                                ...
                                <div className="flex items-center justify-between sm:justify-end gap-4">
                                    <p className={`text-2xl font-black ${selectedPlan === 'rental' ? 'text-white' : 'text-slate-300'} inline`}>
                                        {rentPrice.toLocaleString('vi-VN')}<p className="text-lg underline underline-offset-2 ml-0.5 inline">đ</p>
                                    </p>
