...
import { FaFilm, FaSearch, FaTh, FaList, FaPlay, FaHeart } from 'react-icons/fa';
    ...
    return (
        <div className="w-full flex flex-col gap-6 p-1 sm:p-2">
            ...
            <div className="flex flex-col sm:flex-row items-center justify-between gap-4 mb-6">
                <div className="flex items-center gap-2 text-2xl font-bold uppercase tracking-wider drop-shadow-[0_0_8px_rgba(255,255,255,0.3)] animate-fade-in-favorites">
                    <FaHeart className="text-yellow-400 drop-shadow-[0_0_15px_rgba(250,204,21,0.6)] animate-pulse" />
                    <span className="glow-text text-white">Phim yêu thích</span>
                </div>
            ...
            {favorites.length === 0 ? (
                <div className="mt-4 w-full min-h-[350px] border border-white/5 rounded-3xl bg-[#15171a] flex flex-col items-center justify-center p-8 text-center relative overflow-hidden shadow-inner animate-fade-in-favorites" style={{ animationDelay: '0.2s', opacity: 0 }}>
                ...
                <div className={`mt-4 ${viewMode === 'grid' ? 'grid grid-cols-2 sm:grid-cols-3 md:grid-cols-4 lg:grid-cols-5 gap-6' : 'flex flex-col gap-4'}`}>
                    {filteredMovies.length > 0 ? filteredMovies.map((movie, index) => (
                        viewMode === 'grid' ? (
                            <Link to={`/detailFilm/${movie.id}`} key={movie.id} className="group relative flex flex-col gap-3 cursor-pointer animate-fade-in-favorites" style={{ animationDelay: `${0.1 + index * 0.05}s`, opacity: 0 }}>
                                <div className="relative rounded-2xl overflow-hidden border-[3px] border-transparent bg-slate-800/40 hover:border-yellow-400 transition-all duration-300 hover:shadow-[0_12px_25px_rgba(250,204,21,0.3)] hover:-translate-y-2 aspect-video w-full">
                            ...
                            <div key={movie.id} className="flex flex-col sm:flex-row items-center gap-4 p-3 rounded-2xl border border-white/10 bg-slate-800/50 backdrop-blur-md hover:border-yellow-500/40 hover:shadow-[0_0_25px_rgba(250,204,21,0.2)] transition-all duration-300 group animate-fade-in-favorites" style={{ animationDelay: `${0.1 + index * 0.05}s`, opacity: 0 }}>
                                <Link to={`/detailFilm/${movie.id}`} className="w-full sm:w-40 md:w-48 h-auto aspect-video rounded-xl overflow-hidden shrink-0 border-[2px] border-transparent group-hover:border-yellow-400 transition-all duration-300 relative block">
