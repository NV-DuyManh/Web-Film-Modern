                        ...
                        viewMode === 'grid' ? (
                            <Link to={`/detailFilm/${movie.id}`} key={`grid-${movie.id}`} className="group relative flex flex-col gap-3 cursor-pointer">
                                <div className="relative rounded-2xl overflow-hidden border-[3px] border-transparent bg-slate-800/40 hover:border-blue-400 transition-all duration-300 hover:shadow-[0_12px_25px_rgba(59,130,246,0.3)] hover:-translate-y-2 aspect-2/3 w-full">
                                    ...
                                    <button onClick={(e) => handleRemoveContinue(movie.id, e)} className="absolute top-3 right-3 bg-red-500/80 hover:bg-red-500 text-white p-2 rounded-xl backdrop-blur-md opacity-0 group-hover:opacity-100 transition-all duration-300 hover:shadow-[0_0_15px_rgba(239,68,68,0.5)] z-10" title="Xóa lịch sử">
                                        <FaTrash size={12} />
                                    </button>
                                </div>
                            ...
                            <div key={`list-${movie.id}`} className="flex flex-col sm:flex-row items-center gap-4 p-3 rounded-2xl border border-white/10 bg-slate-800/50 backdrop-blur-md hover:border-blue-500/40 hover:shadow-[0_0_25px_rgba(59,130,246,0.2)] transition-all duration-300 group">
                                <Link to={`/detailFilm/${movie.id}`} className="w-32 sm:w-40 md:w-48 h-auto aspect-video rounded-xl overflow-hidden shrink-0 border-[3px] border-transparent group-hover:border-blue-400 transition-all duration-300 relative block">
                                ...
                                <div className="flex items-center shrink-0 sm:ml-auto w-full sm:w-auto justify-end pr-2 gap-3">
                                    <Link to={`/play/${movie.id}`} className="flex items-center gap-2 bg-linear-to-r from-blue-600 to-indigo-600 hover:from-blue-500 hover:to-indigo-500 text-white px-5 py-2 rounded-xl font-bold transition-all duration-300 hover:shadow-[0_0_20px_rgba(59,130,246,0.5)] hover:scale-105 border border-blue-400/50 text-sm">
                                        <FaPlay size={12} /> Tiếp tục xem
                                    </Link>
                                    <button onClick={(e) => handleRemoveContinue(movie.id, e)} className="p-2.5 rounded-xl bg-slate-700/50 text-slate-400 hover:text-red-500 hover:bg-red-500/10 border border-transparent hover:border-red-500/40 transition-all duration-300 hover:shadow-[0_0_15px_rgba(239,68,68,0.3)]" title="Xóa lịch sử">
                                        <FaTrash size={14} />
                                    </button>
                                </div>
                            </div>
