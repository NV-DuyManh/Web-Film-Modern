...
function ContinueFilm(props) {
    ...
    return (
        <div className="w-full flex flex-col gap-6 p-1 sm:p-2">
            ...
            {continueMovies.length === 0 ? (
                <div className="mt-4 w-full min-h-87.5 border border-white/5 rounded-3xl bg-[#15171a] flex flex-col items-center justify-center p-8 text-center relative overflow-hidden shadow-inner">
                    ...
                    {filteredMovies.length > 0 ? filteredMovies.map(movie => (
                        viewMode === 'grid' ? (
                            ...
                            <div key={movie.id} className="flex flex-col sm:flex-row items-center gap-4 p-3 rounded-2xl border border-white/10 bg-slate-800/50 backdrop-blur-md hover:border-blue-500/40 hover:shadow-[0_0_25px_rgba(59,130,246,0.2)] transition-all duration-300 group">
                                <Link to={`/detailFilm/${movie.id}`} className="w-full sm:w-40 md:w-48 h-auto aspect-video rounded-xl overflow-hidden shrink-0 border-2 border-transparent group-hover:border-blue-400 transition-all duration-300 relative block">
