                    ...
                    <div className="flex flex-row items-center justify-between sm:justify-end gap-3 w-full md:w-auto shrink-0">
                        ...
                        <div className="flex items-center bg-slate-800/80 p-1.5 rounded-xl border border-white/10 shadow-[0_4px_20px_rgba(0,0,0,0.5)] backdrop-blur-md shrink-0 h-10.5">
                            ...
                            <button
                                ...
                                className={`p-1.5 sm:p-2 rounded-lg transition-all duration-300 flex items-center justify-center ${viewMode === 'list' ? 'bg-blue-500/20 text-blue-400 border border-blue-500/50 shadow-[0_0_15px_rgba(59,130,246,0.3)] scale-105' : 'text-slate-400 hover:text-white hover:bg-white/5 border border-transparent'}`}
                            >
                                <FaList size={18} />
                            </button>
                        </div>
                    ...
                    {filteredMovies.length > 0 ? filteredMovies.map(movie => (
                        viewMode === 'grid' ? (
                            ...
                            <div key={`list-${movie.id}`} className="flex flex-col sm:flex-row items-center gap-4 p-3 rounded-2xl border border-white/10 bg-slate-800/50 backdrop-blur-md hover:border-blue-500/40 hover:shadow-[0_0_25px_rgba(59,130,246,0.2)] transition-all duration-300 group">
                                <Link to={`/phim/${movie.slug || movie.id}`} className="w-32 sm:w-40 md:w-48 h-auto aspect-video rounded-xl overflow-hidden shrink-0 border-[3px] border-transparent group-hover:border-blue-400 transition-all duration-300 relative block">
