...
export default function ModalCategoryType({ open, onChangeInput, handleClose, addCategoryType, error, loading, progress, categoryType }) {
    return (
        <Dialog
            open={open}
        ...
        >
            <DialogTitle className="modal-header-x flex justify-between items-center">
                <span className="glow-text-multi text-2xl md:text-3xl font-black tracking-tight" style={{ paddingBottom: '0.1em' }}>
                    {categoryType.id  ? "Update Category Type" : "Add New Category Type"}
                </span>
                <button 
                    onClick={handleClose}
