...
function ModalAuthors({ open, onChangeInput, handleClose, addauthor, error, loading, progress, author, handleImageChange, setAuthor }) {
    const [uploadMode, setUploadMode] = React.useState('file');

    const handleUrlChange = (e) => {
        const url = e.target.value;
        if (setAuthor) setAuthor(pre => ({ ...pre, imgUrl: url, imgFile: null }));
        ...
        >
            ...
            <DialogContent className="modal-body-x">
                ...
                <div className="w-full flex flex-col items-center mt-2 mb-2">
                    <p className="gender-label mb-2 inline">Author Avatar</p>
                    ...
                    <div className="flex flex-col items-center w-full min-h-40 justify-start">
                        {uploadMode === 'file' ? (
                            <div className="relative w-36 h-36 rounded-full border-2 border-dashed border-slate-600 hover:border-cyan-400 overflow-hidden group transition-all duration-300 shadow-[0_0_15px_rgba(0,0,0,0.5)] bg-slate-900/50 flex items-center justify-center">
                                <img 
                                    src={author.imgFile ? URL.createObjectURL(author.imgFile) : getSafeEntityAvatar(author.imgUrl, author.sexID)} 
                                    alt="Author Avatar" 
                                    className="w-full h-full object-cover transition-all duration-500 group-hover:opacity-30" 
                                    onError={(e) => { e.target.onerror = null; e.target.src = getDefaultAvatar(author.sexID); }}
                                />
                            ...
                            <div className="w-full flex flex-col gap-4 items-center">
                                ...
                                <div className="w-36 h-36 rounded-full overflow-hidden border border-white/10 bg-slate-900/50 flex items-center justify-center">
                                    <img src={getSafeEntityAvatar(author.imgUrl, author.sexID)} className="w-full h-full object-cover" alt="Preview" onError={(e) => { e.target.onerror = null; e.target.src = getDefaultAvatar(author.sexID); }} />
                                </div>
                            </div>
                        )}
                    </div>
