...
function ModalActor({ open, onChangeInput, handleClose, addactor, error, loading, progress, actor, handleImageChange, setActor }) {
    const [uploadMode, setUploadMode] = React.useState('file');

    const handleUrlChange = (e) => {
        const url = e.target.value;
        if (setActor) setActor(pre => ({ ...pre, imgUrl: url, imgFile: null }));
        ...
        >
            ...
            <DialogContent className="modal-body-x">
                ...
                <div className="w-full flex flex-col items-center mt-2 mb-2">
                    <p className="gender-label mb-2 inline">Actor Avatar</p>
                        ...
                        {uploadMode === 'file' ? (
                            ...
                            <div className="w-full flex flex-col gap-4 items-center">
                                ...
                                <div className="w-36 h-36 rounded-full overflow-hidden border border-white/10 bg-slate-900/50 flex items-center justify-center">
                                    <img src={getSafeEntityAvatar(actor.imgUrl, actor.sexID)} className="w-full h-full object-cover" alt="Preview" onError={(e) => { e.target.onerror = null; e.target.src = getDefaultAvatar(actor.sexID); }} />
                                </div>
                            </div>
                        )}
                    </div>
