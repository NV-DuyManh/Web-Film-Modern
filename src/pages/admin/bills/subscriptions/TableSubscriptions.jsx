...
function TableSubscriptions({ handleClickOpen, setSubscription, subscription, search }) {
    ...
    const handleDeleted = async () => {
        await deleteDocument("Subscriptions", subscription);
    ...
    const handleBulkDeleted = async () => {
        await Promise.all(
            selectedIds.map(id => {
                const item = subscriptions.find(c => c.id === id);
                return item ? deleteDocument("Subscriptions", item) : Promise.resolve();
            })
        );
    ...
    return (
        <div className="p-5">
            ...
            <ModalDelete
                ...
                titleDelete={"DELETE SUBSCRIPTION"}
                contentDelete={`Are you sure you want to delete this subscription record?`}
            />
            <ModalDelete
